...
async function assertPageTitleEvents(browser) {
  ...
  try {
    ...
    assert.strictEqual(
      await page.title(),
      "Basalt",
      "page should start with Basalt fallback title",
    );

    await page.evaluate(() =>
      window.__seedEspPing({ title: "Basalt 7inch P4" }),
    );
    assert.strictEqual(
      await page.title(),
      "Basalt 7inch P4",
  ...
  try {
    ...
    await normalPage.evaluate(() =>
      window.__seedEspPing({ title: "Basalt 4inch S3" }),
    );
    assert.strictEqual(
...
async function runCase(browser, testCase) {
  ...
  page.on("request", (request) => {
    const requestUrl = new URL(request.url());
    if (
      ["cdn.jsdelivr.net", "fonts.googleapis.com"].includes(
        requestUrl.hostname,
      )
    ) {
...
async function assertPanelNaming(browser) {
  ...
  try {
    ...
    await page.waitForFunction(() => document.title === "Basalt — Kitchen");
    assert.strictEqual(await page.locator(".sp-brand").textContent(), "Basalt Kitchen");
    ...
    assert.strictEqual(await page.title(), "Basalt — Kitchen");
    identityState.failSave = false;
    await save.click();
    await page.waitForSelector("dialog[open]");
    await page.waitForFunction(() => document.title === "Basalt — Office");
    ...
    await page.getByRole("button", { name: "Restore", exact: true }).click();
    await page.waitForFunction(() => document.title === "Basalt — Bedroom");
