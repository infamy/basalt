...
export function runCameraMotionSettingsTests(): void {
  ...
  equal(plainModes, "disabled,timer,sensor", "panels without a camera do not offer Camera Motion mode");
  const cameraModes = screensaverModeOptions(true).map((option) => option[0]).join(",");
  equal(cameraModes, "disabled,timer,sensor,camera", "camera panels offer Camera Motion last");

  equal(screensaverModeForDevice("camera", true), "camera", "camera panels restore Camera Motion mode");
  equal(screensaverModeForDevice("camera", false), "timer", "other panels restore Camera Motion as Timer");
  ...
  equal(restored.screensaverMode, "camera", "backups keep Camera Motion mode");
