...
description:
  How to use lock cards on your Basalt panel to lock, unlock, and view Home Assistant lock entities.
---
...
Basalt does not store lock PINs or codes on the panel. If your lock requires a code, create a Home Assistant script that handles the code securely, then use an [Action](/card-types/actions) card to run that script.
