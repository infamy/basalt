...
description:
  How to use slider cards on your Basalt panel to control lights, fans, and number entities from Home Assistant.
---
...
`number.*` entities normally belong to a device or integration, while `input_number.*` entities are helpers created in Home Assistant. Basalt supports both and sends the matching Home Assistant action automatically.
