...
description:
  How to recreate the ESPHome Media Player cover-art, playback-control, and clock screensaver experience with Basalt.
---

# Migrate from ESPHome Media Player

If you used [ESPHome Media Player](https://jtenniswood.github.io/esphome-media-player/) as a dedicated music controller, Basalt can recreate the main experience while also giving you cards for the rest of Home Assistant.

The two projects do not share their saved settings. Before changing firmware, note the Home Assistant `media_player` entity used by the old display. You will select the same entity in Basalt after following the [Install guide](/getting-started/install) and [enabling Home Assistant actions](/getting-started/home-assistant-actions).

## Choose the Experience You Want

| If you want... | Use... |
| --- | --- |
| Normal Basalt cards until media starts, then a full-screen now-playing display | [Automatic cover art](#option-1-automatic-cover-art) |
...
This is the closest match to the old media-player behaviour. Your normal Basalt cards remain available, but artwork takes over the screen when the selected player starts playing.
...
When Home Assistant supplies a track duration, Basalt also shows playback time and a progress bar. Live radio and other streams without a duration still show their available artwork and track details.

Touching the artwork returns you to the normal card screen. Basalt waits for the selected **Show After** time from your most recent touch before showing the artwork again, so you have time to use other controls.
...
If **Cover Art** or **Extra Large (3x3)** is missing, update the panel to current Basalt firmware and reload its web page.

See [Media cards](/card-types/media#cover-art) for the other card sizes and Cover Art settings.

## Match the Old Idle Screen

Automatic cover art and the normal Basalt screensaver are separate. This lets the panel remain awake while media plays, then switch to a clock or turn off after you stop using it.

1. Open **Settings > Sleep & Schedule > Screensaver**.
2. Choose **Timer**.
3. Choose how long Basalt should wait after the last touch.
...
| ESPHome Media Player | Basalt equivalent |
| --- | --- |
...
Basalt uses taps and cards instead of the old player's swipe gestures. The **All Controls** popup is the simplest replacement for play/pause, previous, next, and volume controls in one place. You can also add separate [Media cards](/card-types/media) for any control you want to keep directly on the home screen.
