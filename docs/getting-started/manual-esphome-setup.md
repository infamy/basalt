---
title: "Install Basalt with ESPHome Device Builder"
titleTemplate: :title
description:
  How to add Basalt to ESPHome manually, compile the firmware, and install it by USB or OTA.
---

# Install Basalt with ESPHome Device Builder

The normal [browser install](/getting-started/install) is the easiest route. Use this page if you prefer to manage Basalt from ESPHome, want to compile the firmware yourself, or need to install from the ESPHome Device Builder dashboard.
...
Use USB for a blank screen or a screen that is not already running Basalt. Once Basalt is installed and connected to WiFi, later ESPHome installs can usually be done wirelessly with OTA.
:::
...
packages:
  api_encryption:
    url: https://github.com/infamy/basalt/
...
Basalt's built-in web page can be protected with a username and password when you compile and install the firmware yourself. This is useful if other people can reach your local network and you do not want them opening the display setup page.
...
Then point the Basalt web auth substitutions at those secrets and add the `web_server_auth` package to your Basalt device YAML:
...
After saving, validate the device and install the firmware again. The next time you open the display address in a browser, it will ask for the username and password. Basalt uses Digest authentication so the password is not sent directly across your network.
...
The `disable_updates: "true"` substitution removes Basalt's built-in GitHub firmware update checker and update controls. ESPHome OTA stays enabled, so you can still install firmware manually once the display is online.
...
Because the package uses `refresh: 1sec`, ESPHome checks GitHub for Basalt updates each time it compiles. To update manually, open ESPHome Device Builder and run **Install** again. If the display is online, use OTA so you do not need to reconnect USB.
