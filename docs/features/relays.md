...
description:
  How Basalt exposes built-in panel relays in Home Assistant.
---
...
Basalt exposes each relay in two ways:
...
They appear as normal Home Assistant entities on the Basalt device. You can control them from Home Assistant dashboards, automations, scripts, and Home Assistant voice assistants.
...
Home Assistant may adjust the exact entity ID if you have renamed the device or if another entity already used the same name. To find them, open **Settings > Devices & services**, select your Basalt device, then look for the relay switches and relay push buttons under the device entities.
...
1. Open the Basalt setup page in your browser.
...
If you need a relay to return to a particular state after restart, handle that with a Home Assistant automation once the Basalt device reconnects.
...
Use the relay feature at your own risk. Basalt and its maintainers are not responsible or liable for damage, injury, code violations, incorrect wiring, equipment failure, or other losses caused by using the relay outputs.
:::
