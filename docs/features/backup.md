...
description:
  How to export and import your Basalt panel configuration as a backup file.
---
...
Backup files are versioned so newer Basalt releases can keep importing older backups safely. Older version 1 backups still import, and new exports use version 2 while keeping the same readable layout fields for compatibility.

## Compatibility Notes

Basalt keeps old saved card strings readable during upgrades. That means cards created before newer card options or compact subpage storage were added should still load, display, and export correctly after an update.
...
When importing a backup from a different-sized panel, Basalt keeps the saved card order where it can and rearranges cards that no longer fit the target screen. Subpages are moved with their parent card when the parent card is kept.
