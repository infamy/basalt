---
title: Immich Photos
description: Display your Immich photo library on Basalt with the Home Assistant companion, including albums, memories, photo pairing and slideshow controls.
---

# Immich Photos

Bring your photo library into everyday view. EspControl Immich Companion connects **Immich**, your self-hosted photo library, to **Home Assistant**, turning selected photos into a slideshow for your Basalt display or Home Assistant dashboard.
...
Check the [installation status](/immich/installation#installation-status) before starting; the companion's development and releases are separate from Basalt firmware.

1. Open **HACS → ⋮ → Custom repositories**. Add `https://github.com/infamy/basalt-immich` as an **Integration**.
...
6. Follow [Connect to Basalt](/immich/display-setup) to use the frame's **Image** entity in the photo screensaver or a Camera card.

Each frame starts with individual photos, landscape output, the full image visible and a 30-second timer. Add the integration again to create more frames. The companion supplies the image and controls; Basalt displays the image through Home Assistant.
...
|---|---|
| [Connect to Basalt](/immich/display-setup) | Screensaver, image sizing, metadata and Camera card setup |
...
Update through HACS and restart Home Assistant; frame settings are kept. Upgrading from the older cache format requires Immich to be reachable for the first new image. See [updates and outages](/immich/using-your-frame#updates-and-outages) or [report a problem](https://github.com/infamy/basalt-immich/issues).

For contributors: [architecture and development](https://github.com/infamy/basalt-immich/blob/18ed9ecd30e6ce16fd07bbe6c34e1f72a67acb15/docs/architecture.md), [entity reference](/immich/entities) and [compatibility](/immich/compatibility).

---

Adapted from the [EspControl Immich Companion documentation](https://github.com/infamy/basalt-immich/blob/18ed9ecd30e6ce16fd07bbe6c34e1f72a67acb15/README.md), used under its [MIT license](/immich-license.txt).
